...
static void maybe_combat_roll(HeroState& h,
                              const common::Action& a,
                              bool aim_consumed,
                              const MatchConfig& config) {
    if (aim_consumed || !a.ability_1 || !h.alive || h.cd_ability_1 != 0)
        return;
    common::Vec2 dir{};
    const float move_mag_sq = a.move_x * a.move_x + a.move_y * a.move_y;
    if (move_mag_sq > 1e-6F) {
        const float inv = 1.0F / std::sqrt(move_mag_sq);
        dir = {a.move_x * inv, a.move_y * inv};
    } else {
        dir = {std::cos(h.aim_angle), std::sin(h.aim_angle)};
    }
...
void stage_abilities_combat_roll(const TickContext& ctx) {
    for (std::uint32_t i = 0; i < kAgentsPerMatch; ++i) {
        ...
        if (!h.present || h.kind != common::HeroKind::Ranger)
            continue;
        maybe_combat_roll(h, ctx.actions[i], ctx.aim_consumed[i], ctx.config);
    }
}

void stage_abilities_ranger_mark_target(const TickContext& ctx) {
    for (std::uint32_t i = 0; i < kAgentsPerMatch; ++i) {
        HeroState& ranger = ctx.state.heroes[i];
        if (!ranger.present || !ranger.alive || ranger.kind != common::HeroKind::Ranger)
            continue;
        const auto& a = ctx.actions[i];
        if (ctx.aim_consumed[i] || !a.ability_2 || a.target_slot != 1 || ranger.cd_ability_2 != 0)
            continue;
        int best_slot =
            nearest_enemy_in_range_with_los(ctx.state, i, common::kRangerRevolverRange, ctx.config);
        ranger.cd_ability_2 = common::kRangerMarkTargetCooldownTicks;
        if (best_slot >= 0) {
            HeroState& target = ctx.state.heroes[(std::uint32_t)best_slot];
