...
void stage_vanguard_warhammer(const TickContext& ctx, DamageBuffer& buf,std::array<bool, kAgentsPerMatch>& has_damage){ for(std::uint32_t i=0;i<kAgentsPerMatch;++i){HeroState& v=ctx.state.heroes[i]; if(!v.present||!v.alive||v.kind!=common::HeroKind::Vanguard) continue; const auto& a=ctx.actions[i]; if(ctx.aim_consumed[i]||!a.primary_fire||v.vanguard_barrier_active||v.weapon.fire_cooldown_ticks>0) continue; int best_slot=nearest_enemy_in_cone_with_los(ctx.state,i,common::kVanguardWarhammerRange,common::kVanguardWarhammerHalfAngleCos,ctx.config); v.weapon.fire_cooldown_ticks=common::kVanguardWarhammerCooldownTicks; if(best_slot>=0){buf[i].victim_slot=(std::uint32_t)best_slot;buf[i].damage_centi_hp=(std::uint32_t)common::kVanguardWarhammerDamageCentiHp;has_damage[i]=true;}} }
