...
def make_xushi_vector_env(
    ...
    *,
    ...
    if backend == "sim_pool":
        # The sim_pool backend reads env parameters back off the envs-layer
        # factory partials, which this phase-agnostic layer must not import.
        raise ValueError(
            "vector backend 'sim_pool' is built by "
            "envs.runtime_factory.make_vector_env, not by this function"
        )
    raise ValueError(
