from __future__ import annotations

import numpy as np

from envs.phase5_entity_mappo import Phase5EntityMappoEnv
from xushi2.entity_obs import (
    ENTITY_OBS_DIM,
    ENTITY_TOKEN_COUNT,
    ENTITY_TOKEN_DIM,
    actor_obs_to_entity_obs,
    entity_obs_self_position,
)
from xushi2.obs_manifest import ACTOR_PHASE1_DIM, CRITIC_DIM, actor_field_slice


def _make_sim_cfg(round_length: int = 5) -> dict:
    return {
        "seed": 0xD1CEDA7A,
        "round_length_seconds": round_length,
        "fog_of_war_enabled": False,
        "randomize_map": False,
        "action_repeat": 3,
        "mechanics": {
            "revolver_damage_centi_hp": 7500,
            "revolver_fire_cooldown_ticks": 15,
            "revolver_hitbox_radius": 0.75,
            "respawn_ticks": 240,
        },
    }


def test_actor_obs_to_entity_obs_shapes_mask_and_self_position() -> None:
    obs = np.zeros((2, ACTOR_PHASE1_DIM), dtype=np.float32)
    obs[:, actor_field_slice("own_position")] = np.array(
        [[0.25, -0.5], [-0.1, 0.3]], dtype=np.float32
    )
    obs[0, actor_field_slice("enemy_alive")] = 1.0
    obs[1, actor_field_slice("enemy_alive")] = 0.0

    entity_obs = actor_obs_to_entity_obs(obs)
    assert entity_obs.shape == (2, ENTITY_OBS_DIM)

    mask = entity_obs[:, ENTITY_TOKEN_COUNT * ENTITY_TOKEN_DIM :]
    np.testing.assert_array_equal(mask[0], np.array([1.0, 1.0, 1.0]))
    np.testing.assert_array_equal(mask[1], np.array([1.0, 0.0, 1.0]))
    np.testing.assert_allclose(
        entity_obs_self_position(entity_obs), obs[:, actor_field_slice("own_position")]
    )


def test_phase5_env_returns_entity_obs_and_phase4_critic_obs() -> None:
    env = Phase5EntityMappoEnv(_make_sim_cfg(), opponent_bot="noop")
    try:
        obs, info = env.reset(seed=0)
        assert obs.shape == (3, ENTITY_OBS_DIM)
        assert obs.dtype == np.float32
        assert info["learner_team"] == "A"

        critic_obs = np.zeros(CRITIC_DIM, dtype=np.float32)
        env.build_critic_obs(critic_obs)
        assert np.all(np.isfinite(critic_obs))

        next_obs, reward, term, trunc, _ = env.step(
            np.zeros((3, 6), dtype=np.float32)
        )
        assert next_obs.shape == (3, ENTITY_OBS_DIM)
        assert reward.shape == (3,)
        assert isinstance(term, bool)
        assert isinstance(trunc, bool)
    finally:
        env.close()
