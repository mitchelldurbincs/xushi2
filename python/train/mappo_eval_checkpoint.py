...
def maybe_run_bc_pretrain(context: RuntimeContext, trainer: MappoTrainer, logger: Any) -> bool:
    ...
    if bc_variant == "walk_and_shoot":
        ...
        if aim_rehearsal_batch_size > 0:
            ...
            mini_game_cfg = dict(target_raw.get("config", {}).get("env", {}).get("mini_game_config", {}))
    ...
    print(f"[{context.phase_label}/mappo] bc_eval mean_reward={eval_stats.mean_reward:+.3f} "
          f"wins={eval_stats.wins}/{eval_stats.episodes} draws={eval_stats.draws}/{eval_stats.episodes} "
          ...
          f"same_tgt={eval_stats.team_a_same_target_fraction:.3f}/{eval_stats.team_b_same_target_fraction:.3f} "
          f"focus_H={eval_stats.team_a_target_selection_entropy:.3f}/{eval_stats.team_b_target_selection_entropy:.3f} "
    ...
    passed = eval_stats.team_a_hit_fire > min_hit_fire and eval_stats.team_a_aim_error_rad < max_aim_error
    print(f"[{context.phase_label}/mappo] bc_combat_gate passed={passed} "
          f"team_a_hit_fire={eval_stats.team_a_hit_fire:.4f}>{min_hit_fire:.4f} "
          f"team_a_aim_error={eval_stats.team_a_aim_error_rad:.3f}<{max_aim_error:.3f}", flush=True)
